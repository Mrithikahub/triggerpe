...
def detect_fraud(worker_id, trigger_type, amount, location, gps_lat=None, gps_lng=None):
    ...
    if duplicate_exists(worker_id, trigger_type):
        score += 0.50; flags.append("DUPLICATE_CLAIM_TODAY")

    if claim_freq >= 4:
        score += 0.25; flags.append(f"HIGH_FREQUENCY_{claim_freq}_IN_7D")

    if gps_lat and gps_lng and not _gps_valid(location, gps_lat, gps_lng):
        score += 0.40; flags.append("LOCATION_MISMATCH")
    elif not gps_lat:
        score += 0.10; flags.append("NO_GPS")

    if amount > 500:
        score += 0.20; flags.append("ABNORMAL_AMOUNT")
